...
export default function FinalOutputsStep({ onPrev, onDataUpdate, claimData }: FinalOutputsStepProps) {
  ...
  const generateFinalOutputs = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8001/api/generate-final-outputs', {
        method: 'POST',
        headers: {
  ...
  const downloadClaimPacket = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8001/api/download-final-pdf/${claimData.claimPacket.claim_id}`);
      ...
      toast.success('Final validated claim packet downloaded!');
    } catch (error) {
      ...
      {finalOutputs && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Final Claim Packet PDF */}
            <div className="border rounded-lg p-6">
              <div className="flex items-center mb-4">
                <Download className="h-6 w-6 text-blue-600 mr-2" />
                <h3 className="font-medium text-gray-900">Final Claim Packet PDF</h3>
              </div>
              <p className="text-sm text-gray-600 mb-4">
                Cleaned and validated claim packet with AI Judge results
              </p>
              <button
            ...
            <div className="border rounded-lg p-6">
              <div className="flex items-center mb-4">
                ...
                <h3 className="font-medium text-gray-900">Cryptographic Proof Card</h3>
              </div>
              <div className="space-y-2 text-sm">
                <div>
                  <span className="text-gray-600">Claim Hash:</span>
                  <p className="font-mono text-xs break-all">{finalOutputs.proof_card?.claim_hash}</p>
                </div>
                <div>
                  <span className="text-gray-600">ECDSA Signature:</span>
                  <p className="font-mono text-xs break-all">{finalOutputs.proof_card?.ecdsa_signature?.substring(0, 32)}...</p>
                </div>
                <div>
                  <span className="text-gray-600">Timestamp:</span>
                  <p className="text-gray-900">{new Date(finalOutputs.proof_card?.timestamp * 1000).toLocaleString()}</p>
                </div>
              </div>
              <a
