...
class EigenCloudJudge {
    constructor(mnemonic) {
        ...
        this.account = (0, crypto_1.createHash)('sha256').update(mnemonic).digest('hex').slice(0, 42);
    ...
    async evaluateClaim(claimData) {
        ...
        for (const rule of this.constitution.rules.completeness) {
            ...
            results.rules_evaluated.push(evaluation);
    ...
    calculateConfidence(rules) {
        const avgConfidence = rules.reduce((sum, rule) => sum + rule.confidence, 0) / rules.length;
        return avgConfidence || 0;
    }
    generateRationale(results) {
        const passedRules = results.rules_evaluated.filter((r) => r.passed).length;
    ...
    generateAttestationHash(claimData) {
        const attestationHash = (0, crypto_1.createHash)('sha256')
    ...
    async signEvaluation(results) {
        // Sign the evaluation results with the TEE private key
        const messageHash = (0, crypto_1.createHash)('sha256').update(JSON.stringify(results)).digest('hex');
    ...
    static verifyAttestation(evaluation, expectedAddress) {
